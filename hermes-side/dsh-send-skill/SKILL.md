---
name: dsh-send
description: "把任务下发给 DSH（DeepSeek Harness Web 端），可带模型/设置选项。用法：/dsh-send [--model M] [--provider P] 任务内容。DSH 会在 Web GUI 自动执行并在看板回写结果。"
version: 1.2.3
---
...
## 流程
1. 解析用户指令里的选项（`--model`、`--provider`、`--priority`、`--max-runtime`、`--workspace dir:<path>`、`--idempotency-key <key>`、`--skill <name>`），未指定则用 Hermes 当前默认模型。
...
4. 用 `hermes kanban create` 创建任务（看板 `dsh`，assignee `dsh`，模型/设置原样传入）：
   ```bash
   hermes kanban --board dsh create "<任务内容>（输出要求：在 DSH 工作区内新建合适命名文件夹存放，路径写进结果）【参考技能】$DSH_WORKSPACE/skills-shared/<name>/" \
   ...
   ```
   > ⚠️ 实测（2026-08-14）：`--model/--provider` 存为 `model_override/provider_override` 字段；create 后任务初始状态为 `ready`（非 todo）。**`create` 无 `--output-dir` 参数**（DSH 审查确认，2026-08-14）——落点必须并入任务正文，不能作为 CLI flag 传。
   > ⚠️ 会话继承（已设计，实施中，尚未可用）：后续将支持 `--resume <关键词>` 让 DSH 继承历史会话上下文；当前版本任务正文携带 session id 是人工参考，非自动机制。
5. **写触发文件**（事件驱动唤醒 watcher）：`echo <task_id> > ~/.dsh/kanban-trigger/<task_id>.trigger`（目录不存在则 `mkdir -p`）。
6. 把返回的任务 id 告诉用户：任务已投递，DSH 的 Web GUI 会自动开始执行；查看结果用 `hermes kanban --board dsh show <id>` 或 `/inbox`。
